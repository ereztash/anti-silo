    ...
    dropzone.addEventListener('drop', event => {
      ...
      if (file && file.path) startScan(file.path);
      else { statusEl.className = 'panel'; statusEl.innerHTML = '<b>הדפדפן לא חשף נתיב תיקייה מלא.</b><br>בגרסת דפדפן רגילה יש להדביק את הנתיב בשדה. באריזת Desktop הפעולה הזו תעבוד כגרירה מלאה.'; }
    });
    if (initialPath) startScan(initialPath);
