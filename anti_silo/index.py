...
from .model import OPERATOR_ATTESTED, SELF_DECLARED, Surface
...
def classify_surface(
    ...
    if not found:
        return None
    # A pointer written by the repair flow records that a human explicitly
    # picked this source; a marker typed into a corpus file does not.
    origin = OPERATOR_ATTESTED if meta.get("type", "") == "user_selected_source" else SELF_DECLARED
    return Surface(
        rel_path,
        tuple(found),
        authorities[0],
...
def write_index(vault: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    with (out / "truth_surface_index.csv").open("w", encoding="utf-8", newline="") as f:
        writer = SafeDictWriter(f, fieldnames=["file", "surfaces", "authority", "can_anchor_claim", "raw_source", "content_hash", "raw_source_hash", "normalized_content_hash", "trust_origin"])
        writer.writeheader()
        for row in rows:
            writer.writerow(
                {
